...
function generateSalt(len) {
  ...
  do{ salt = gen(len); } while(string === 'null')
  ...
  try {
    ...
    for(var i=0; i<iterations; ++i) {
      if (salt !== null){
        ...
        if (algorithm === 'md5') hash = crypto.createHash(algorithm).update(hash).digest('hex');
        else hash = crypto.createHmac(algorithm).update(hash).digest('hex');
      }
...
module.exports.generate = function(password, options) {
  ...
  options.saltLength || options.saltLength === null || options.saltLength == 0 || (options.saltLength = 8);
